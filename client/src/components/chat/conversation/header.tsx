...
import { CallIcon, DotsIcon, SearchLargeIcon, VideoCallIcon } from '../../../icons';
...
export default function ChatHeader({ insOnline, callUser }: ChatHeaderProps) {
  const { activeConversation, conversationTyping } = useSelector(
    (state: AppState) => state.chat,
  );

  const status = useMemo(() => {
    if (conversationTyping.includes(activeConversation?._id || ''))
      return 'Typing...';
  ...
  return (
    <div className="h-[52px] dark:bg-dark_bg_2 flex items-center p16 w-full select-none">
            ...
            <>
              <li>
                <button className="btn" onClick={callUser}>
                  <VideoCallIcon className="dark:fill-dark_svg_1 w-[30px]" />
                </button>
              </li>
              <li>
                <button className="btn">
